...
static const struct drm_display_mode pipa_display_mode = {
...
static struct drm_dsc_config pipa_dsc_cfg = {
	.dsc_version_major = 1,
	.dsc_version_minor = 1,
	.slice_height = 20,
	.slice_width = 450,
...
static const struct panel_desc pipa_csot_desc = {
	...
	.display_mode = pipa_display_mode,
	.dsc_cfg = &pipa_dsc_cfg,
...
static int nt36532_prepare(struct drm_panel *panel)
{
	...
	drm_dsc_pps_payload_pack(&pps, ctx->desc->dsc_cfg);
					...
					struct drm_connector *connector)
{
	...
	return drm_connector_helper_get_modes_fixed(connector, &desc->display_mode);
}
...
static int nt36532_probe(struct mipi_dsi_device *dsi)
{
	...
	for (i = 0; i < ARRAY_SIZE(ctx->dsi); i++) {
		...
		ctx->dsi[i]->dsc = ctx->desc->dsc_cfg;
